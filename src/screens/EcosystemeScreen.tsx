...
const HEALTH_LABEL_EN: Record<HealthLabel, string> = {
  Bon: 'Good',
  Vigilance: 'Watch',
  Critique: 'Critical',
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.headerBlock}>
        <Text style={styles.title}>{t('Santé environnementale', 'Environmental health')}</Text>
        <Text style={styles.subtitle}>{t('Cette semaine · votre quartier', 'This week · your neighborhood')}</Text>
      </View>
      ...
      <HealthCard dashboard={dashboard} loading={loading} styles={styles} colors={colors} t={t} />
      ...
      <Text style={styles.privacy}>
        {t(
          ...
          `Aggregated data only. The index is only shown from ${dashboard.privacyThreshold} signals onward.`,
        )}
      </Text>
    </ScrollView>
  );
      ...
      <View>
        {loading ? (
          ...
          <Text style={styles.healthLabel}>
            {t(dashboard.healthLabel, HEALTH_LABEL_EN[dashboard.healthLabel])}
          </Text>
        )}
        <Text style={styles.healthCaption}>
          {dashboard.healthScore === null
            ? t('Confidentialité préservée', 'Privacy preserved')
            : t(`Indice de santé local · ${dashboard.healthScore}/100`, `Local health index · ${dashboard.healthScore}/100`)}
        </Text>
      </View>
